...
<div class="row bg-cover">
    ...
    <div class="col-lg-12 text-center mt-3">
        <span><a href={{ action([\App\Http\Controllers\VendorController::class, 'index']) }}>{{ html()->img('images/vendor.png', 'Vendor Index')->attribute('title', "Vendor Index")->class('btn btn-outline-dark') }}</a></span>
    ...
    <div class="col-lg-12 mt-3">
                    ...
                    <li>
                        @can('delete-relationship')
                            {{ html()->form('DELETE', route('relationship.destroy', [$a_relationship->id]))->attribute('onsubmit', 'return ConfirmDelete()')->open() }}
                                ...
                                {{ html()->image('images/delete.png', 'btnDelete', ['title' => 'Delete Relationship ' . $a_relationship->id, 'style' => 'padding-left: 50px;']) }}
                            {{ html()->form()->close() }}
                        @else
                            {!!$vendor->contact_link!!} {{ $a_relationship->relationship_type->label_a_b }} {!! $a_relationship->contact_b->contact_link !!}
                        @endCan
                ...
                @foreach($vendor->b_relationships as $b_relationship)
                    <li>
                        @can('delete-relationship')
                            ...
                            {{ html()->image('images/delete.png', 'btnDelete', ['title' => 'Delete Relationship ' . $b_relationship->id, 'style' => 'padding-left: 50px;']) }}
                            {{ html()->form()->close() }}
                        @else
                            {!!$vendor->contact_link!!} {{ $b_relationship->relationship_type->label_b_a }} {!! $b_relationship->contact_a->contact_link!!}
                        @endCan
        ...
        <div class="row">
            <div class="col-lg-6 text-right">
                @can('update-contact')
                    <a href="{{ action([\App\Http\Controllers\VendorController::class, 'edit'], $vendor->id) }}" class="btn btn-info">{{ html()->img('images/edit.png', 'Edit')->attribute('title', "Edit") }}</a>
                @endCan
            </div>
