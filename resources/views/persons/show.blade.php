    ...
    <div class="col-lg-12">
        <div class="row text-center">
            ...
            <div class="col-lg-12 mt-2">
                @can('show-group')
                @if ($person->is_board_member) <span><a href={{ action([\App\Http\Controllers\PersonController::class, 'boardmembers']) }}>{{ html()->img('images/board.png', 'Board Members Group')->attribute('title', "Board Members Group")->class('m-1 btn btn-outline-dark') }}</a></span> @endIf
                @if ($person->is_ambassador) <span><a href={{ action([\App\Http\Controllers\PersonController::class, 'ambassadors']) }}>{{ html()->img('images/ambassador.png', 'Ambassador Group')->attribute('title', "Ambassadors Group")->class('m-1 btn btn-outline-dark') }}</a></span> @endIf
                @if ($person->is_staff) <span><a href={{ action([\App\Http\Controllers\PersonController::class, 'staff']) }}>{{ html()->img('images/employee.png', 'Staff Group')->attribute('title', "Employees Group")->class('m-1 btn btn-outline-dark') }}</a></span> @endIf
                @if ($person->is_steward) <span><a href={{ action([\App\Http\Controllers\PersonController::class, 'stewards']) }}>{{ html()->img('images/steward.png', 'Steward Group')->attribute('title', "Stewards Group")->class('m-1 btn btn-outline-dark') }}</a></span> @endIf
                @if ($person->is_volunteer) <span><a href={{ action([\App\Http\Controllers\PersonController::class, 'volunteers']) }}>{{ html()->img('images/volunteer.png', 'Volunteers Group')->attribute('title', "Volunteers Group")->class('m-1 btn btn-outline-dark') }}</a></span> @endIf
                @if ($person->is_retreat_director) <span><a href={{ action([\App\Http\Controllers\PersonController::class, 'directors']) }}>{{ html()->img('images/director.png', 'Retreat Directors Group')->attribute('title', "Directors Group")->class('m-1 btn btn-outline-dark') }}</a></span> @endIf
                @if ($person->is_retreat_innkeeper) <span><a href={{ action([\App\Http\Controllers\PersonController::class, 'innkeepers']) }}>{{ html()->img('images/innkeeper.png', 'Retreat Innkeepers Group')->attribute('title', "Innkeepers Group")->class('m-1 btn btn-outline-dark') }}</a></span> @endIf
                @if ($person->is_retreat_assistant) <span><a href={{ action([\App\Http\Controllers\PersonController::class, 'assistants']) }}>{{ html()->img('images/assistant.png', 'Retreat Assistants Group')->attribute('title', "Assistants Group")->class('m-1 btn btn-outline-dark') }}</a></span> @endIf
                @if ($person->is_bishop) <span><a href={{ action([\App\Http\Controllers\PersonController::class, 'bishops']) }}>{{ html()->img('images/bishop.png', 'Bishops Group')->attribute('title', "Bishop Group")->class('m-1 btn btn-outline-dark') }}</a></span> @endIf
                @if ($person->is_pastor) <span><a href={{ action([\App\Http\Controllers\PersonController::class, 'pastors']) }}>{{ html()->img('images/pastor.png', 'Pastors Group')->attribute('title', "Pastors Group")->class('m-1 btn btn-outline-dark') }}</a></span> @endIf
                @if ($person->is_priest) <span><a href={{ action([\App\Http\Controllers\PersonController::class, 'priests']) }}>{{ html()->img('images/priest.png', 'Priests Group')->attribute('title', "Priests Group")->class('m-1 btn btn-outline-dark') }}</a></span> @endIf
                @if ($person->is_deacon) <span><a href={{ action([\App\Http\Controllers\PersonController::class, 'deacons']) }}>{{ html()->img('images/deacon.png', 'Deacons Group')->attribute('title', "Deacons Group")->class('m-1 btn btn-outline-dark') }}</a></span> @endIf
                @if ($person->is_provincial) <span><a href={{ action([\App\Http\Controllers\PersonController::class, 'provincials']) }}>{{ html()->img('images/provincial.png', 'Provincials Group')->attribute('title', "Provincials Group")->class('m-1 btn btn-outline-dark') }}</a></span> @endIf
                @if ($person->is_superior) <span><a href={{ action([\App\Http\Controllers\PersonController::class, 'superiors']) }}>{{ html()->img('images/superior.png', 'Superiors Group')->attribute('title', "Superiors Group")->class('m-1 btn btn-outline-dark') }}</a></span> @endIf
                @if ($person->is_jesuit) <span><a href={{ action([\App\Http\Controllers\PersonController::class, 'jesuits']) }}>{{ html()->img('images/jesuit.png', 'Jesuits Group')->attribute('title', "Jesuits Group")->class('m-1 btn btn-outline-dark') }}</a></span> @endIf
                @endCan
            </div>
            <div class="col-lg-12 mt-2">
          ...
          <div class='col-lg-6  mt-4 mb-4'>
            @can('update-contact')
                <a href="{{ action([\App\Http\Controllers\PersonController::class, 'edit'], $person->id) }}" class="m-1 btn btn-info mr-4">
                  {{ html()->img('images/edit.png', 'Edit')->attribute('title', "Edit") }}
                </a>
            @endCan

            @can('delete-contact')
                {{ html()->form('DELETE', route('person.destroy', [$person->id]))->attribute('onsubmit', 'return ConfirmDelete()')->class('d-inline')->open() }}
                {{ html()->image('images/delete.png', 'btnDelete', ['class' => 'm-1 btn btn-danger', 'title' => 'Delete']) }}
                {{ html()->form()->close() }}
            @endCan
        </div>
      </div>
